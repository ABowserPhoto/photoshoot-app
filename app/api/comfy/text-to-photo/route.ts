  ...
  try {
    ...
    if (!promptNode?.inputs || !samplerNode?.inputs) {
      return NextResponse.json(
        { error: "Text-to-photo workflow template missing required nodes: 4 or 6." },
        { status: 500 }
      );
    }
    ...
    const message = error instanceof Error ? error.message : "Failed to trigger text-to-photo workflow.";
